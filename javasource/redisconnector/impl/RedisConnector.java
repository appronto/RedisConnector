...
import redisconnector.proxies.Enum_Sort;
import redisconnector.proxies.GeoPosition;
import redisconnector.proxies.InputRow;
import redisconnector.proxies.ResultRow;
...
public class RedisConnector {
	...
	public RedisConnector() {
		try {
			...
			throw new IllegalArgumentException(
					"Please configure constant RedisDatabaseIndex with value 0 or higher, current value: "
							+ redisconnector.proxies.constants.Constants.getRedisDatabaseIndex().trim());
		}
	}
	...
	public String rpoplpush(String source, String destination) {
		try {
			...
			setDatabase();
			_logNode.debug("rpoplpush source: " + source + " destination: " + destination);
	...
	public String brpoplpush(String source, String destination, int timeout) {
		try {
			redis = pool.getResource();
			setDatabase();
			_logNode.debug("brpoplpush source: " + source + " destination: " + destination + " timeout: " + timeout);
			...
			new Thread(new Runnable() {
				...
				public void run() {
					try {
						...
						subscriberRedis.select(Integer
								.valueOf(redisconnector.proxies.constants.Constants.getRedisDatabaseIndex().trim()));
	...
	public java.util.List<IMendixObject> zrange(IContext context, String Key, long Start, long Stop,
			redisconnector.proxies.Enum_Sort Sort) {
		try {
			redis = pool.getResource();
			setDatabase();
	...
	public IMendixObject hmget(IContext context, String Key, IMendixObject ObjectToReturn) throws Exception {
		try {
			...
			for (String result : resultList) {
				...
				if (m instanceof MendixDateTime) {
					ObjectToReturn.setValue(context, fields[i],
							processDateString(context, result, ((MendixDateTime) m).shouldLocalize()));
					i++;
					continue;
				}
				if (m instanceof MendixInteger) {
	...
	private static Date processDateString(IContext context, String value, boolean shouldLocalize) {
		...
		DateFormat dateFormat = DateFormat.getDateTimeInstance(DateFormat.SHORT, DateFormat.DEFAULT, userLocale);
